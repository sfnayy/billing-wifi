...
export default function AdminReports() {
  ...
  const generatePDF = () => {
    ...
    const tableRows = mockTransactions.map(trx => [
        trx.id, trx.date, trx.name, `Rp ${trx.amount.toLocaleString('id-ID')}`, trx.status
    ]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <button onClick={generatePDF} className="flex items-center gap-2 px-6 py-3 bg-slate-800 hover:bg-slate-900 text-white rounded-xl shadow-md transition-colors">
          ...
          <tbody>
            {mockTransactions.map((trx, idx) => (
              <tr key={idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                <td className="py-4 px-6 font-medium text-slate-700 flex items-center gap-2"><FileText size={16} className="text-slate-400" />{trx.id}</td>
                ...
                <td className="py-4 px-6">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${trx.status === 'Lunas' ? 'bg-emerald-100 text-emerald-700' : trx.status === 'Tertunda' ? 'bg-amber-100 text-amber-700' : 'bg-rose-100 text-rose-700'}`}>
                    {trx.status}
                  </span>
                </td>
              </tr>
