...
function StatCard({ title, amount, icon, trend }) {
  ...
  return (
    <div className="glass rounded-3xl p-6 shadow-sm border border-slate-200/60">
      <div className="flex justify-between items-start mb-4">
        <div className="p-3 rounded-2xl bg-white shadow-sm border border-slate-100">{icon}</div>
        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${isPositive ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>{trend}</span>
      </div>
      <div>
